...
describe("GiftsReducer_Component", () => {
  ...
  it("should handle FETCH_CARD", () => {
    ...
    expect(giftsReducer(INITIAL_STATE, fetch_card).giftCard).toBe(
      getGiftCardData
    );
  });
  it("should handle FETCH_CARDS", () => {
    ...
    expect(giftsReducer(INITIAL_STATE, fetch_cards).giftCards.length).toEqual(
      1
    );
    expect(
      giftsReducer(INITIAL_STATE, fetch_cards).giftCardsFiltered.length
    ).toEqual(1);
  });
  ...
  it("should handle UPDATE_CARD_COUNT", () => {
    ...
    expect(giftsReducer(INITIAL_STATE, fetch_cards).giftCard.cardCount).toEqual(
      95
    );
  });
});
